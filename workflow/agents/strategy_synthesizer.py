...
def _build_v3_1_prompt(
    ...
    # LLM 출력 스키마 힌트(최소 구조)
    schema_hint = {
        "summary": "전략 핵심 요약 (구체적이고 실행 가능한 방향성 제시 - 내부 역량, 경쟁사 대응, 시장/정책 관점을 명확히 구분하여 작성)",
        "focus": {
...
**전략 요약 작성 가이드:**
summary 필드는 다음 3개 관점으로 구체적으로 작성하세요:
1. 내부 역량 관점: SK AX의 기존 기술/레퍼런스를 어떻게 활용할 것인지 구체적 방향
2. 경쟁사 대응 관점: 경쟁사 강점/약점에 대한 구체적 대응 전략
3. 시장/정책 관점: 시장 트렌드나 정부 정책을 어떻게 활용할 것인지 구체적 방향

예시 형식:
"본 전략은 RFP 요구사항에 대한 적합도를 기반으로 [구체적 내부 역량 활용 방안]을 통해 [구체적 기술적/사업적 강화점]을 달성하고, [경쟁사명]의 [구체적 강점]에 대응하여 [구체적 차별화 전략]을 제시합니다. 특히 [구체적 시장/정책 활용 방안]을 통해 [구체적 성과]를 목표로 합니다."
...
def _generate_deal_brief(strategy: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "■ 전체 전략 요약 보기",
        "",
        _shorten(summary, 800),
        "",
        "## 2) 전략 포커스",
        "",
        f"◇ 내부 역량 관점",
        f"{_shorten(focus.get('internal','-'), 500)}",
        "",
        f"▶ 경쟁사 대응 관점",
        f"{_shorten(focus.get('competitor','-'), 500)}",
        "",
        f"▶ 시장/정책 관점",
        f"{_shorten(focus.get('market','-'), 500)}",
        "",
        "## 3) 경쟁사 분석",
